...
namespace DDBHelper
{
    public class DynamoDBHTTP : MonoBehaviour
    {
        ...
        void Start()
        {
            // you'll need to build your own JSON for requests, but... here's a very very simple on
            string s = "{ \"TableName\": \"DDBHelper\" }";
            SendWWWRequest(s);
        }
        ...
        void SendWWWRequest(string payload)
        {
...
#else
            // Setup the request url to be sent to DynamoDB
            WWW www = new WWW(endpoint, Encoding.UTF8.GetBytes(payload), headers);
#endif
        ...
        /// </summary>       
        static string ToHex(byte[] data, bool lowercase)
        {
            StringBuilder sb = new StringBuilder();
